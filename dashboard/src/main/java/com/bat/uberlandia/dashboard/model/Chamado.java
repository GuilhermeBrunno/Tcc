...
public class Chamado {
    ...
    public long getTempoReparoSegundos(){
        ...
        if ((status == Status.PAUSADO || status == Status.ESCALADO) && inicioPausa != null) {
            return acumulado;
        }
    LocalDateTime referencia = (inicioPausa != null)
    ? inicioPausa : inicioReparo;
 ...
 public boolean isAtrasado(){
    if (status != Status.EM_ANDAMENTO && status != Status.ESCALADO)
        return false;
    return getTempoReparoSegundos() > 1800;
